...
public class PessoaServiceImpl extends CrudServiceImpl<Pessoa, Long> implements PessoaService {
	...
	private boolean valida(SolicitacaoCadastroViewModel solicitacao, ResultadoOperacaoViewModel<Pessoa> result) {
		if (solicitacao.getTipoPessoa() == TipoPessoa.Fisica) {
			...
			if (!solicitacao.isEhProfessor()
					&& (solicitacao.getOrientador() == null || solicitacao.getOrientador().getId() == 0)) {
	...
	public SolicitacaoCadastroViewModel getDadosValidacaoCadastro(long idSolicitacaoCadastro) {
		...
		var solicitacaoView = new SolicitacaoCadastroViewModel(solicitacao.getId(), solicitacao.getTipoPessoa(),
				solicitacao.getNome(), solicitacao.getCep(), solicitacao.getEndereco(), solicitacao.getBairro(),
				solicitacao.getNumero(), solicitacao.getComplemento(), solicitacao.getTelefone(),
				solicitacao.getCelular(), solicitacao.getEmail(), solicitacao.getCidade(), solicitacao.getDataCriacao(),
				solicitacao.getDocumento(), solicitacao.isEhProfessor(), solicitacao.getNomeInstituicao(),
				solicitacao.getNomeProgramaEnsino(), solicitacao.getDataTerminoProgramaEnsino(),
				solicitacao.getCpfOrientador(), solicitacao.getNomeprojeto());

		if (solicitacao.getNomeInstituicao() != null && !solicitacao.getNomeInstituicao().isEmpty()) {
			solicitacaoView.setInstituicao(instituicaoRepository.findByNome(solicitacao.getNomeInstituicao()));
		}

		if (solicitacao.getNomeProgramaEnsino() != null && !solicitacao.getNomeProgramaEnsino().isEmpty()) {
			solicitacaoView.setProgramaEnsino(programaEnsinoRepository
					.findByNomeOrSigla(solicitacao.getNomeProgramaEnsino(), solicitacao.getNomeProgramaEnsino()));
		}

		if (solicitacao.getTipoPessoa() == TipoPessoa.Fisica && !solicitacao.isEhProfessor()) {
			solicitacaoView
					.setOrientador(pessoaInstituicaoService.findByInstituicaoNomeAndPessoaDocumentoAndEhProfessorTrue(
							solicitacao.getNomeInstituicao(), StringUtils.getNumbers(solicitacao.getCpfOrientador())));
	...
	public ResultadoOperacaoViewModel findCpfCnpjCadastrado(TipoPessoa t, String cpfCnpj) {
		...
		if (!exists) {
			return result.returningFailMessage(
					t == TipoPessoa.Fisica ? ValidationMessages.CpfNaoExiste : ValidationMessages.CnpjNaoExiste);
		}
	...
	public void sendEmailRetornoSolicitacao(String emailTo) {
		Email email = new Email().setTitulo("Retorno Solicitação de Cadastro").setPara(emailTo)
