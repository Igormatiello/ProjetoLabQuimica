...
public class FormularioController extends CrudController<Formulario, Long> {
    ...
    public ResultadoOperacaoViewModel mudaStatus(@PathVariable Long id, @RequestBody @Valid FormularioAnaliseViewModel analise){
        return formularioService.analisaFormulario(analise.setFormularioId(id));
    }
    
 // UPLOAD
 	@PostMapping("upload/{id}")
	public ResponseEntity<?> save(@PathVariable Long id, @RequestParam("arquivos") MultipartFile[] arquivos, HttpServletRequest request){
