...
public class SolicitacaoCadastroRepositoryTests {
	...
	public void deveDeletarUmaSolicitacaoCadastro() {
		...
		SolicitacaoCadastro solicatacaoCadastroDeletada = entityManager.find(SolicitacaoCadastro.class,
				solicitacaoCadastro.getId());
	...
	public void deveBuscarUmaSolicitacaoCadastro() {
		...
		java.util.Optional<SolicitacaoCadastro> solicitacaoCadastroEncontrada = repository
				.findById(solicitacaoCadastro.getId());
	...
	public void deveAtualizarUmaSolicitacaoCadastro() {
		...
		SolicitacaoCadastro solicitacaoCadastroNovo = entityManager.find(SolicitacaoCadastro.class,
				solicitacaoCadastro.getId());
	...
	public static SolicitacaoCadastro criarSolicitacaoCadastro() {
		...
		return SolicitacaoCadastro.builder().senha("123").tipoPessoa(TipoPessoa.Fisica).cidade(cidade).nome("igor")
				.documento("100").cep("8986000").endereco("rua dos marajos").bairro("centro").numero("877")
				.complemento("ed solymar").nomeprojeto("analise quimica peixe").telefone("898989").celular("898989")
				.email("igor@email.com").dataCriacao(LocalDate.now()).nomeInstituicao("utfpr").cpfOrientador("1010")
				.nomeProgramaEnsino("nome").ehProfessor(false).dataTerminoProgramaEnsino(LocalDate.of(2022, 10, 4))
				.inscricaoEstadual("ie").pessoa(pessoa).build();
